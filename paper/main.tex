...
% decoding accuracy by number of components:

%   - richer conditions have higher peak accuracy (ttests between pairs of
%   conditions). implies that richer conditions evoke more stable brain patterns
%   across people.  (Fig 2A)

%   - richer conditions have steeper initial slopes: intact condition reaches a
%   threshold of 5% decoding accuracy faster than paragraph-scrambled (ttest) or
%   word-scrambled (ttest), and decoding accuracy never exceeds 5% in the rest
%   condition. implies richer conditions are more compressible, such that the
%   same number of components can be more informative.  (Fig 2B)

%   - decoding accuracy appears to assymptote beyond some number of components.
%   used sigmoid procedure to find the inflection point where the curve flattens
%   out (Fig 2C). this represents a "balance" point between higher decoding
%   accuracy (generally better for more components) and compression (better for
%   fewer components). these inflection points change systematically across
%   conditions (num components and decoding accuracy; Fig 2D)
