...
For example, the extent that activity patterns at one brain area can be
...
that unreliability could be compensated for by other region's activity patterns
that are predictive of the unreliable region.
...
compressibility, our work suggests a potential reason for why the statistical
...
features are relevant~\citep{MackEtal20}.
...
Fig.~\ref{fig:networks}). This might reflect evolutionary optimizations that
reflect the relevant constraints or demands placed on those networks. One
possibility is that cortex is organized in a hierarchy of networks concerned
with different levels of processing or function. To the extent that different
levels of processing (e.g., low-level sensory processing versus ``deeper''
higher-level processing) reflect different stimulus
timescales~\citep[e.g.,][]{Mann20}, the network differences we observed might
also relate to the timescales ot which each network is maximally
...
activity patterns and the associated cognitive representations and computations
are linked. We identified two aspects of brain activity patterns,
